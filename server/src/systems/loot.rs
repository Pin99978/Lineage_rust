use bevy::prelude::*;
use shared::{GroundItem, Inventory, Position};
...
pub fn loot_system(
    ...
    mut players: Query<
        (&Position, &mut Inventory, &network::NetworkEntity),
        With<network::PlayerCharacter>,
    >,
    items: Query<(Entity, &network::NetworkEntity, &GroundItem, &Position)>,
    mut despawn_messages: MessageWriter<ItemDespawnedMessage>,
    mut inventory_messages: MessageWriter<InventoryUpdateMessage>,
) {
    for request in requests.read() {
        let Ok((player_position, mut inventory, player_network)) =
            players.get_mut(request.looter_entity)
        else {
            continue;
        };

        let Some((item_entity, item_network, ground_item, item_position)) = items
            .iter()
            .find(|(_, network_entity, _, _)| network_entity.id == request.item_id)
        else {
            continue;
        };
